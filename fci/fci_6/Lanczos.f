        ...
        double precision v_l(4,4,4,4,4,4)
     ...
     &     dabs(SpinSubSpace(i,j,k,l,m,n)-n_sz).GT.1.0d-1)THEN
        ...
        Engformer=-10.0d0; EngNow=0.0d0
        v_j_1=0.0d0;

        DO jj = 2, 100

         beta_j_1 = beta(jj-1)
         v_j = hv * ( 1.0d0/beta_j_1 )
         v_buf(:,:,:,:,:,:,jj) = v_j(:,:,:,:,:,:)
...
c          write(*,*)krylovh
          ...
          EngNow=EigLanc(1)
c          write(*,*)eiglanc(1:6)+core_energy

          IF((Dabs(EngNow-Engformer).LT.1.0d-10).OR.(jj.GE.100))THEN
           if( jj .ge. 8)then
            iroot = 0
            do ii = 1, 10
             v_x=0.0d0
...
c             if( (dabs( num_a + num_b - nele ) .le. 1.0d-1 ).and.
c     &           (dabs( s_z - n_sz ) .le. 1.0d-1 ) .and. 
c     &           (dabs( s_sq - n_s_sq ) .le. 1.0d-1 ) .and. 
c     &           iroot .lt. 3)then
              iroot = iroot + 1
...
c          do j = 2, l-1
c          v_i = v_buf(:,:,:,:,:,:,j)
c          vu=ddot(4096,v_i,1,v_l,1)
c          uu=ddot(4096,v_i,1,v_i,1)
c          call daxpy(4096, -1.0d0*vu/uu, v_i,1,
c     &                                   v_l,1)
c          enddo
c          v_buf(:,:,:,:,:,:,l) = v_l
c         enddo
c         do l = 2, jj
c          call normalise( v_buf(:,:,:,:,:,:,l) )
c         enddo
